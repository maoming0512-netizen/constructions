...
import { Leaf, Mail, Lock, Eye, EyeOff, Github } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.error) {
        // 登录成功，短暂延迟等待 session 更新
        setTimeout(() => {
          // 检查邮箱是否为管理员（因为此时 session 可能还未更新）
          if (email === '279364248@qq.com') {
            router.push('/admin')
          } else {
            router.push('/')
          }
      ...
      setErrors({ general: error?.message || 'Login failed. Please check database connection.' })
    } finally {
      setIsLoading(false)
    }
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            Continue exploring English constructions.
          </motion.p>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4" style={{ marginTop: 'var(--space-8)' }}>
            ...
            <div className="flex items-center justify-between mt-1">
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="remember" className="text-body-sm" style={{ color: 'var(--soft-gray)' }}>
                  Remember me
                </label>
              </div>
              <Link
                href="#"
                className="text-body-sm transition-all duration-[var(--duration-fast)] hover:underline"
