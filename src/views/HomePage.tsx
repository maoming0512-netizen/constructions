    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            Here, you don't practice isolated grammar rules — you practice the real &ldquo;form-meaning pairings&rdquo; that exist in the English language.
          </motion.p>
      ...
      <section style={{ background: 'var(--warm-white)', padding: 'var(--space-24) 0' }}>
        <div className="max-w-[var(--container-max)] mx-auto px-4 sm:px-6">
          <SectionHeader
            eyebrow="EXERCISE TYPES"
            title="Three Ways to Practice English Writing"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { icon: <FileText className="w-6 h-6" />, title: 'Micro Continuation (D1)', desc: 'Read a short story opening and write the next ~120 words. Practice narrative flow, character development, and using constructions to express cause, effect, and emotion.', color: '#6BA3BE' },
              { icon: <BookOpen className="w-6 h-6" />, title: 'Long Continuation (D2)', desc: 'Read a longer passage and continue it in 150–300 words. Develop deeper narrative coherence, paragraph-level organization, and varied sentence structures.', color: '#8AB89A' },
              { icon: <Globe className="w-6 h-6" />, title: 'Chinese-English Translation (T1)', desc: 'Translate Chinese sentences into natural English. Learn to avoid word-for-word translation by identifying the right construction for each meaning.', color: '#B8A9C9' },
            ].map((item, i) => (
              <motion.div
                key={item.title}
                ...
                whileHover={{ y: -2 }}
                className="group rounded-[var(--border-radius-md)] p-6 transition-all duration-[var(--duration-normal)]"
                style={{ border: '1px solid var(--glass-border)', background: 'rgba(255,255,255,0.70)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)' }}
              >
                <div className="w-12 h-12 rounded-full flex items-center justify-center mb-4 transition-transform duration-[var(--duration-fast)] group-hover:scale-110"
                  style={{ background: `${item.color}18`, color: item.color }}>
                  {item.icon}
                </div>
                <h4 className="text-h4 text-[--deep-slate] mb-2">{item.title}</h4>
                <p className="text-body-sm text-[--soft-gray] leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
          </div>
          ...
          >
            <button
              ...
              style={{ background: 'var(--lake-green)', boxShadow: '0 4px 16px rgba(138,184,154,0.25)' }}
            >
            ...
            {[
              {
                ...
                title: "D1 & D2 Continuation Writing",
                desc: 'Practice story continuation with 210 exercises across Junior High and Senior High levels. Read a passage, then write the next part — just like the Gaokao exam.',
              },
              {
                icon: <FileText className="w-8 h-8" />,
                title: "Chinese-English Translation",
                desc: 'Translate Chinese sentences into natural English. Learn to avoid word-for-word translation and express meaning like a native speaker.',
              },
              {
                icon: <Bot className="w-8 h-8" />,
                title: "AI-Powered Constructive Feedback",
                desc: 'Submit your answer and receive detailed analysis: what constructions you used well, what could be improved, and a rewritten elevated version of your work.',
              },
              {
                icon: <BarChart3 className="w-8 h-8" />,
                title: "Track Your Progress",
                desc: 'Every answer you write is saved. View your history, word count trends, and band score progression over time.',
              },
              {
                icon: <GraduationCap className="w-8 h-8" />,
                title: "Built on Construction Grammar",
                desc: 'Grounded in Adele Goldberg\'s research. Constructions are form-meaning pairings — learn to think in patterns, not memorize rules.',
              },
              {
                icon: <FileCheck className="w-8 h-8" />,
                title: "AI-Generated Exercises",
                desc: 'Use the AI Lab to generate custom writing exercises tailored to your level and preferred topic. Save them to your personal exercise bank.',
              },
            ].map((item, i) => (
              <motion.div
                key={i}
                ...
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true, amount: 0.15 }}
                transition={{ duration: 0.6, delay: i * 0.12, ease: easeGentle }}
                className="rounded-[var(--border-radius-lg)] p-7"
                style={{
              ...
              >
                <div className="mb-4" style={{ color: 'var(--lake-blue)' }}>{item.icon}</div>
                <h4 className="text-h4 text-[--deep-slate] mb-2">{item.title}</h4>
                <p className="text-body-sm text-[--soft-gray] leading-relaxed">{item.desc}</p>
              </motion.div>
            ))}
      ...
      >
        <div className="max-w-[var(--container-narrow)] mx-auto px-4 sm:px-6 text-center">
          ...
          >
            Join students and teachers using Construction Grammar to improve their English writing. Free to start.
          </motion.p>
